                    ...
                    <div className="w-full max-w-4xl mx-auto px-4 py-8 text-center">
                        ...
                        <div className="flex justify-center gap-2 sm:gap-3 md:gap-4 mb-10">
                            {[{ value: timeLeft.days, label: 'DIAS' }, { value: timeLeft.hours, label: 'HORAS' }, { value: timeLeft.minutes, label: 'MIN' }, { value: timeLeft.seconds, label: 'SEG' }].map((item, i) => (
                                <div key={i} className="bg-white/10 rounded-lg px-3 py-2 md:px-5 md:py-3 min-w-[64px] md:min-w-[90px]">
                                    <div className="text-2xl md:text-4xl font-black text-habilitar-orange">{formatNumber(item.value)}</div>
                                    <div className="text-[10px] md:text-xs font-bold text-gray-300 tracking-wider">{item.label}</div>
                                </div>
                            ))}
                        </div>

                        <div className="flex flex-col items-center gap-6">
                            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="bg-green-500 hover:bg-green-600 text-white font-bold px-8 md:px-12 py-4 md:py-6 rounded-full transition-all duration-300 transform hover:scale-105 text-lg md:text-2xl shadow-xl shadow-green-500/30 flex items-center gap-3" onClick={() => window.fbq && window.fbq('track', 'Lead')}>
                            ...
                            <p className="text-gray-400 text-sm animate-pulse">
                                Clique acima para garantir seu acesso pelo WhatsApp.
                            </p>
                        </div>
                    </div>
                </div>
